...
## 客户端
实习是鸡架，工作是后端，之前没有关注过几种常见的基础软件的图形化客户端。
后面因为有一种VSCode All In One的执念使用VSCode的插件，后面实在有一些不方便的地方，专业的事情还是要交给专业的人。

+ HTTP
    + VSCode插件：Thunder Client，基本功能是没问题的，但是自带的编辑窗口“不能用”（有错位）
...
+ MySQL：
    ...
    + DBeaver：叔叔推荐的
+ NoSQL：
    + VSCode插件：Database Client，这个插件好像是专门给关系型数据库用的，只是也支持NoSQL，功能非常勉强啊！比较搜索键，比如对一个键相关信息的图形化展示，拉。
    + TinyRDM：开源干净好用。
