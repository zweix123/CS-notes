+ [Unexpected security footguns in Go's parsers Go](https://blog.trailofbits.com/2025/06/17/unexpected-security-footguns-in-gos-parsers/)
...
3. 相似名称过多。比如`net/http`中，包对外方法、结构体`ServeMux`都有方法`HandleFunc`，还有一个类型也是`HandleFunc`。
    >不知道能不能通过最佳实践解决
4. 构造结构体时类似C而非使用类似C++的构造函数，会出现若干参数我没有设置也不会报错，这在多人合作时，假如结构体的定义者增加了字段，但是结构体的使用者不修改也能编译，则容易出现bug。（要么就是接口（结构体字段）设计前相兼容）
    >golang相关的lint应该可以解决这个问题，即假如某些字段没有使用则会提示/报错。但是目前感觉不好用，不能保证流程的完全，个人认为这种情况还是容易出现的。

5. 擦，这么东西怎么能过编译啊！在我看来这种你编译期报错才对啊，万恶的interface{}
```go
package main
...
8. [Golang语言本身的"20/80"法则](https://blog.kowalczyk.info/article/d-2025-06-26/go-is-8020-language.html)：用20%的复杂性去换取80%的实用性，我这样理解，假如希望去实现100%的实用性，可能需要120%的复杂性，即随着复杂性的提升，实用性提升的边际效应递减。我一定程度赞成这样的观点，可以想象当我从word转移到markdown上的时候，不过Golang的复杂性到不了80%吧，我个人就感觉只有50%/60%。
...
+ Ref：
    ...
    10. https://nghiant3223.github.io/2025/04/15/go-scheduler.html
