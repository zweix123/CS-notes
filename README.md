...
+ 要不要写代码解读呢？
    + 对于笔者以及能力超过笔者的人受益不大
        + 笔者：通过阅读已经完成对代码架构的理解，落在笔头看似有帮助重构的好处，但是当重新拾起代码时，仍然选择看源码而非笔记。
        + 能力超过笔者的读者：

## 规范
>写笔记的规范

+ 引用：使用`[]()`即时引用，文章开头的Ref无序列表表示归纳的比较多的资料。
+ 画图：
    + 流程图：
    + 模块图：
    + 时序图：
...
我写文章的宗旨是，写没有人写过的，或者写比别人写的好的。在这样的宗旨下，如果有一个我想聊的话题，已经有足够好的资料，我会提供链接。
...
+ Golang：
	...
	+ 设计模式：https://www.yuque.com/aceld/lfhu8y，为什么一个语言需要单独的设计模式呢？因为Golang这种interface的形式我个人确实不太习惯。书不错。
+ Rust：
